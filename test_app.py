...
from models import Event, EventCancellation, Signup, User
...
class ComedyOpenMicTestCase(unittest.TestCase):
    ...
    def test_comedian_signup_for_event(self):
        ...
        comedian_id = self.create_test_user(
            username="comedian", email="comedian@example.com"
        )
        ...
        rv = self.app.post(
            ...
            data=dict(notes="Excited to perform!"),
            follow_redirects=True,
        )
        ...
        with app.app_context():
            signup = Signup.query.filter_by(
                comedian_id=comedian_id, event_id=event_id
            ).first()
    ...
    def test_live_lineup_display(self):
        ...
        # Create host and event
        host = self.create_test_user(
            username="host", email="host@example.com", is_host=True
        )
        event = self.create_test_event(host)

        # Create comedian and signup
        comedian = self.create_test_user(
            username="comedian", email="comedian@example.com"
        )
    ...
    def test_event_cancellation(self):
        ...
        today = date.today()
        rv = self.app.post(
            f"/host/cancel_event/{event_id}",
            data=dict(
                cancelled_date=today.strftime("%Y-%m-%d"), reason="Venue unavailable"
            ),
            follow_redirects=True,
        )
        ...
        with app.app_context():
            cancellation = EventCancellation.query.filter_by(
                event_id=event_id, cancelled_date=today
            ).first()
    ...
    def test_duplicate_signup_prevention(self):
        ...
        rv = self.app.post(
            f"/comedian/signup/{event_id}",
            data=dict(notes="Second signup attempt"),
            follow_redirects=True,
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_authentication_required(self):
        ...
        protected_routes = ["/dashboard", "/host/create_event", "/host/dashboard"]
