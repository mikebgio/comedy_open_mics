...
from models import Event, Signup as OldSignup, EventCancellation, User as OldUser
...
def migrate_data():
    ...
    with app.app_context():
        ...
        print(f"Found {len(old_events)} events, {len(old_signups)} signups, {len(old_cancellations)} cancellations")
        ...
        from models_new import (User, Show, ShowRunner, ShowHost, ShowInstance, 
                               ShowInstanceHost, Signup)
        ...
        for old_event in old_events:
            new_show = Show(
                ...
                started_date=old_event.created_at.date() if old_event.created_at else date.today(),
        ...
        for event_id, instance_date in dates_with_activity:
            if event_id in show_mapping:
                ...
                cancellation = next((c for c in old_cancellations 
