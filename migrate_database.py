...
def migrate_database():
    ...
    from models_backup import User as OldUser, Event as OldEvent, Signup as OldSignup, EventCancellation as OldEventCancellation
    ...
    with app.app_context():
        ...
        from models import User, Show, ShowRunner, ShowHost, ShowInstance, ShowInstanceHost, Signup
        ...
        for event_data in events_data:
            try:
                show = Show(
                    ...
                    started_date=event_data['created_at'].date() if event_data['created_at'] else date.today(),
        ...
        for signup_data in signups_data:
            dates_with_activity.add((signup_data['event_id'], signup_data['event_date']))
        
        for cancellation_data in cancellations_data:
            dates_with_activity.add((cancellation_data['event_id'], cancellation_data['cancelled_date']))
            ...
            if event_data['is_active']:
                for weeks_ahead in range(8):
                    ...
                    for day_offset in range(7):
                        ...
                        if check_date.strftime('%A') == event_data['day_of_week'] and check_date >= today:
        ...
        for event_id, instance_date in dates_with_activity:
            if event_id in show_mapping:
                ...
                try:
                    ...
                    print(f"Error creating instance for show {event_id} on {instance_date}: {e}")
