    ...
    for event in upcoming_events:
        ...
        if today.strftime("%A") == event.day_of_week:
            ...
            cancellation = EventCancellation.query.filter_by(
                event_id=event.id, cancelled_date=today
            ).first()
            if not cancellation:
...
def comedian_dashboard():
    ...
    upcoming_signups = (
        Signup.query.filter(
            Signup.comedian_id == current_user.id, Signup.event_date >= today
        )
        .order_by(Signup.event_date)
        .all()
    )
    ...
    return render_template(
        "comedian/dashboard.html", upcoming_signups=upcoming_signups, events=events
    )
...
def signup_for_event(event_id):
    ...
    existing_signup = Signup.query.filter_by(
        comedian_id=current_user.id, event_id=event_id, event_date=next_event_date
    ).first()
    ...
    cancellation = EventCancellation.query.filter_by(
        event_id=event_id, cancelled_date=next_event_date
    ).first()
    ...
    if form.validate_on_submit():
        ...
        current_signups = Signup.query.filter_by(
            event_id=event_id, event_date=next_event_date
        ).count()
    ...
    return render_template(
        "comedian/signup.html", form=form, event=event, next_date=next_event_date
    )
...
def manage_lineup(event_id):
    ...
    signups = (
        Signup.query.filter_by(event_id=event_id, event_date=event_date)
        .order_by(Signup.position.asc().nullslast(), Signup.signup_time)
        .all()
    )

    return render_template(
        "host/manage_lineup.html", event=event, signups=signups, event_date=event_date
    )
...
def random_select(event_id):
    ...
    unpositioned_signups = Signup.query.filter_by(
        event_id=event_id, event_date=event_date, position=None
    ).all()

    if unpositioned_signups:
        # Get the highest current position
        max_position = (
            db.session.query(db.func.max(Signup.position))
            .filter_by(event_id=event_id, event_date=event_date)
            .scalar()
            or 0
        )
...
def cancel_event(event_id):
    ...
    if form.validate_on_submit():
        ...
        existing = EventCancellation.query.filter_by(
            event_id=event_id, cancelled_date=form.cancelled_date.data
        ).first()
...
def event_info(event_id):
    ...
    recent_cancellations = (
        EventCancellation.query.filter(
            EventCancellation.event_id == event_id,
            EventCancellation.cancelled_date >= date.today(),
        )
        .order_by(EventCancellation.cancelled_date)
        .all()
    )

    return render_template(
        "public/event_info.html", event=event, cancellations=recent_cancellations
    )
...
def live_lineup(event_id):
    ...
    # Check if cancelled
    cancellation = EventCancellation.query.filter_by(
        event_id=event_id, cancelled_date=event_date
    ).first()
        ...
        if current_date >= today:
            for event in events:
                if current_date.strftime("%A") == event.day_of_week:
                    # Check if not cancelled
                    cancellation = EventCancellation.query.filter_by(
                        event_id=event.id, cancelled_date=current_date
                    ).first()

                    if not cancellation:
                        # Get signup count for this date
                        signup_count = Signup.query.filter_by(
                            event_id=event.id, event_date=current_date
                        ).count()
...
def move_event():
    ...
    cancellations = EventCancellation.query.filter_by(
        event_id=event_id, cancelled_date=old_date
    ).all()
