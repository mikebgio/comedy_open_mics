...
def test_dashboard_access(client):
    ...
    client.post("/login", data={"username": "dashuser", "password": "testpass123"})
...
def test_calendar_functionality(client):
    ...
    client.post("/login", data={"username": "caluser", "password": "testpass123"})
...
def test_event_info_pages(client):
    ...
    with app.app_context():
        ...
        instance = ShowInstance(
            show_id=show.id, instance_date=date.today() + timedelta(days=7)
        )
