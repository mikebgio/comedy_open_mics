    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            cancellations_result = conn.execute(
                text("SELECT * FROM event_cancellation")
            )
                ...
                if event["is_active"]:
                    for weeks_ahead in range(8):
                        ...
                        for day_offset in range(7):
                            ...
                            if (
                                check_date.strftime("%A") == event["day_of_week"]
            ...
            # Update sequences
            conn.execute(
                text("SELECT setval('show_id_seq', (SELECT MAX(id) FROM show))")
            )
            conn.execute(
                text(
                    "SELECT setval('show_instance_id_seq', (SELECT MAX(id) FROM show_instance))"
                )
            )
            conn.execute(
                text("SELECT setval('signup_id_seq', (SELECT MAX(id) FROM signup))")
            )
            ...
            instance_count = conn.execute(
                text("SELECT COUNT(*) FROM show_instance")
            ).scalar()
